...
sh blast.sh $input_file $db $threads $max_hits
echo -e "\nBLAST hits obtained. Moving on...\n"
# modify the output to spread the hits that gave the same score
python modBlast.py -i blast.out -o blast_mod.out 

# extract taxids and generate taxonomy lineage from taxids
cut -f 1,3 blast_mod.out > taxids.txt

# Need to modify the db variable for taxonkit
NCBI_nt=$(echo $db | cut -d"/" -f1,2,3,4,5,6) 
grep "OTU" taxids.txt | cut -f 2 | taxonkit reformat -I 1 --threads $threads --data-dir $NCBI_nt > full_lineage.txt

paste blast_mod.out taxids.txt full_lineage.txt > taxformat.txt

cat taxformat.txt | tr "\t" "," | tr ";" "," | cut -d"," -f 1,2,4,5,6,7,11,12,13,14,15,16,17 | sed '1i query,subject,bitscore,e_value,percent_identity,query_coverage,Kingdom,Phylum,Class,Order,Family,Genus,Species' > taxonomy.blast

echo -e "\n>>>> Generating taxonomy file <<<<\n"
echo -e "python getTaxonomy.py -c $confidence -m $max_hits -e $ethresh -p $p_iden_thresh\n"
python getTaxonomy.py $confidence $max_hits $ethresh $p_iden_thresh
