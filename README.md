...
After that, you should install the two necessary softwares: `BLAST` and `taxonkit`. **BlasTAX** is written to use a conda environemnt for this purpose that is called *BLAST*. To generate the conda environment you need to have a version of anaconda or miniconda installed. Please refer to [this](https://docs.anaconda.com/free/anaconda/install/index.html) link. In the `condaEnv/` directory you can find a file called `BLAST.yml` that you can use to regenerate the same environment I used to develop **blasTAX**. __Note__ both *BLAST* and *taxonkit* may be updated in the future so it is up to you if you want to use the newwest versions.
